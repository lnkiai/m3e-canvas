...
- **OpenRouter と無料モデル** – プロバイダーに OpenRouter を追加すると、1 つのキー（クレジットカード不要）でメインモデルと最大 2 つの予備モデルを共有できます。どのモデル欄にも OpenRouter の無料 `:free` モデル ID が使えるので、AI 下書きを無料で試せます。
...
- **OpenRouter 与免费模型** – 把 OpenRouter 加为服务商后，一把密钥（无需信用卡）即可共享主模型和最多两个备份模型。每个模型框都接受 OpenRouter 的免费 `:free` 型号，因此可以零成本体验 AI 草图。
...
- **OpenRouter와 무료 모델** – OpenRouter를 제공업체로 추가하면 키 하나(신용카드 불필요)로 메인 모델과 최대 두 개의 예비 모델을 공유합니다. 모든 모델 칸에 OpenRouter의 무료 `:free` 모델 ID를 넣을 수 있어 AI 초안을 무료로 써 볼 수 있습니다.
