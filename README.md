...
- **AI drafts that always finish** – when a reply hits the output limit in the middle of the document, the partial design is handed to the next model in your backup-models list to complete it; if every model stops, a **Continue** button resends the partial reply with the same request, so the run finishes exactly where it stopped instead of drawing from scratch. You can **stop** a draft or an edit at any time, an **Edit this design** button refines the canvas with one targeted change, and a finished-looking reply that is not valid JSON is asked for once more automatically.
- **OpenRouter and free models** – add OpenRouter as a provider with one key (no credit card needed) and pick the main model plus up to two backup models that share it. Every model field accepts OpenRouter's free `:free` model IDs, so drafting with AI can run at no cost.
...
- **いつでも完成する AI 下書き** – 応答が途中で出力上限を超えたときは、未完成の設計を予備モデルが引き継いで完成させます。どのモデルも途中で止まった場合は **「デザインを続ける」** を押すと、途中まで書かれた応答をそのまま同じリクエストで再送し、止まった位置から仕上げます。下書きや編集はいつでも **停止** でき、**「このデザインを編集」** は現在のキャンバスに望みの変更を 1 つ適用します。読めない JSON のまま終わった応答にも続きを作れます。
- **OpenRouter と無料モデル** – プロバイダーに OpenRouter を追加すると、1 つのキー（クレジットカード不要）でメインモデルと最大 2 つの予備モデルを共有できます。どのモデル欄にも OpenRouter の無料 `:free` モデル ID が使えるので、AI 下書きを無料で試せます。- **補助線スナップ**、Undo/Redo、キーボードショートカット、7 種のカラーテーマ、お気に入り部品。作業内容はブラウザ（localStorage）に自動保存されます。
...
- **不会半途而废的 AI 草图** – 当回复在文档中间撞上输出上限时，未写完的设计会交给备份模型继续完成；若所有模型都中途停下，点 **“继续设计”** 会带着写了一半的回复再次发送同样的请求，从断点继续补完，而不是从头重画。草图和编辑随时可以 **停止**；**“编辑此设计”** 可对当前画布应用一处你想要的修改。连读不成的 JSON 回复也可以继续。
- **OpenRouter 与免费模型** – 把 OpenRouter 加为服务商后，一把密钥（无需信用卡）即可共享主模型和最多两个备份模型。每个模型框都接受 OpenRouter 的免费 `:free` 型号，因此可以零成本体验 AI 草图。- **对齐辅助线**、撤销／重做、键盘快捷键、收藏组件，所有内容自动保存在浏览器（localStorage）中。
...
- **끝까지 완성되는 AI 초안** – 응답이 문서 중간에서 출력 한도에 걸리면 미완성 설계를 예비 모델이 이어받아 완성합니다. 모든 모델이 멈췄다면 **"디자인 계속하기"**를 누르면 중간까지 쓰인 응답으로 같은 요청을 다시 보내, 끊긴 지점부터 마무리합니다. 초안이나 편집은 언제든 **중지**할 수 있고, **"이 디자인 편집"**은 현재 캔버스에 원하는 변경 하나를 적용합니다.
- **OpenRouter와 무료 모델** – OpenRouter를 제공업체로 추가하면 키 하나(신용카드 불필요)로 메인 모델과 최대 두 개의 예비 모델을 공유합니다. 모든 모델 칸에 OpenRouter의 무료 `:free` 모델 ID를 넣을 수 있어 AI 초안을 무료로 써 볼 수 있습니다.- **정렬 안내선**, 실행 취소/다시 실행, 키보드 단축키, 7가지 색상 테마, 즐겨찾기 부품. 작업 내용은 브라우저(localStorage)에 자동 저장됩니다.
